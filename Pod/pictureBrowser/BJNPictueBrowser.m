...
#import "BJNPictureBrowserPrivate.h"
...
@implementation BJNPictueBrowser
...
//    _skipNextPagingScrollViewPositioning = NO;;
    ...
    @synchronized(_visiblePages) {
        for (BJNZoomingScrollView *page in _visiblePages) {
...
- (BJNCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    BJNCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <BJNPicture> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[BJNCaptionView alloc] initWithPhoto:photo];
        }
    }
    return captionView;
}
...
- (void)loadAdjacentPhotosIfNecessary:(id<BJNPicture>)photo {
    BJNZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <BJNPicture> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    BJPBweakifyobject(photo);
                    [photo loadUnderlyingImageProgress:^(CGFloat process) {
                        BJPBstrongifobject(photo);
                    ...
                    BJNLog(@"Pre-loading image at index %lu", (unsigned long)pageIndex-1);
                }
            }
...
- (void)tilePages {
    ...
    @synchronized(_visiblePages)
    {
        for (BJNZoomingScrollView *page in _visiblePages) {
            ...
            if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
                ...
                BJNLog(@"Removed page at index %lu", (unsigned long)pageIndex);
            }
        }
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
            BJNZoomingScrollView *page = [self dequeueRecycledPage];
            if (!page) {
//                page = [[BJZoomingScrollView alloc] initWithPhotoBrowser:self];
                page = [[BJNZoomingScrollView alloc] init];
            ...
            BJNLog(@"Added page at index %lu", (unsigned long)index);
            
            // Add caption
            BJNCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
            if (captionView) {
...
- (BJNZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    ...
    BJNZoomingScrollView *thePage = nil;
    //加锁防止多线程同时访问
    @synchronized(_visiblePages)
    {
        for (BJNZoomingScrollView *page in _visiblePages) {
...
- (BJNZoomingScrollView *)pageDisplayingThumbPhoto:(id<BJNPicture>)thumbPhoto {
...
- (void)configurePage:(BJNZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    id<BJNPicture> picture = [self photoAtIndex:index];
    //原图已经存在，直接展示原图
    ...
    id<BJNPicture> photo = [self photoAtIndex:index];
    page.photo = photo;
    __weak BJNPictueBrowser *weakSelf = self;
    [page setImageDidLoadBlock:^(BJNZoomingScrollView *page, id<BJNPicture> picture) {
        if (picture == photo) {
...
- (BJNZoomingScrollView *)dequeueRecycledPage {
    BJNZoomingScrollView *page = [_recycledPages anyObject];
    if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index > 0) {
        ...
        for (i = 0; i < index-1; i++) {
            ...
            if (photo != [NSNull null]) {
                ...
                BJNLog(@"Released underlying image at index %lu", (unsigned long)i);
            }
        }
    }
    ...
    id <BJNPicture> currentPhoto = [self photoAtIndex:index];
...
- (void)actionButtonPressed:(id)sender {
    if (_actionsSheet) {
        ...
        id <BJNPicture> photo = [self photoAtIndex:_currentPageIndex];
...
- (void)actuallySavePhoto:(id<BJNPicture>)photo {
    if ([photo underlyingImage]) {
...
- (void)actuallyCopyPhoto:(id<BJNPicture>)photo {
    if ([photo underlyingImage]) {
...
- (void)actuallyEmailPhoto:(id<BJNPicture>)photo {
    if ([photo underlyingImage]) {
...
//    id<BJPicture> picture = [self photoAtIndex:_currentPageIndex];
    ...
    if (fromView && [picture imageExist]) {
        self.animator = [[BJNPictureZoomAnimator alloc] init];
        ...
        [picture loadUnderlyingImageOnlyLocal:YES progress:nil completed:^(UIImage *image, NSError *error, BOOL finished) {
            ...
            [self view];
            BJNZoomingScrollView *cell = [self pageDisplayedAtIndex:_currentPageIndex];
            [cell displayImage];
