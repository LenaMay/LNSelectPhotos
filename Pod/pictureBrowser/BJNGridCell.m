...
@interface BJNGridCell () {
    
    UIImageView *_imageView;
    UIImageView *_loadingError;
	BJNDACircularProgressView *_loadingIndicator;
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
		...
		// Loading indicator
		_loadingIndicator = [[BJNDACircularProgressView alloc] initWithFrame:CGRectMake(0, 0, 40.0f, 40.0f)];
...
- (void)setProgressFromNotification:(NSNotification *)notification {
    ...
    id <BJNPicture> photoWithProgress = [dict objectForKey:@"photo"];
...
- (void)handleMWPhotoLoadingDidEndNotification:(NSNotification *)notification {
    id <BJNPicture> photo = [notification object];
    if (photo == _photo) {
