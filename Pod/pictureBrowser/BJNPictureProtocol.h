...
typedef void(^BJNImageLoadProgressBlock)(CGFloat process);

typedef void(^BJNImageLoadCompletedBlock)(UIImage *image, NSError *error, BOOL finished);
...
- (void)loadUnderlyingImageOnlyLocal:(BOOL)flag progress:(BJNImageLoadProgressBlock)progressBlock
                          completed:(BJNImageLoadCompletedBlock)completedBlock ;
